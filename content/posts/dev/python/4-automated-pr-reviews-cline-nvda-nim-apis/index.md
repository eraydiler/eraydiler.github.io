...
Here's an actual PR review generated by the workflow on my **TaxCalculator** project — a large refactor turning a Flask monolith into Clean Architecture boundaries.

### PR Review — eraydiler/TaxCalculator#13

Overall the PR is a large refactor that improves structure (Clean Architecture boundaries, partials, JS modules, tests). However, several **blocking issues** need attention before merging.

#### 🔴 Blocking — Privacy: zero-data-retention claim is violated

`ARCHITECTURE.md` (and `README.md`) explicitly promise *"No user report data (CSV content, tax summary) is persisted on the server. All report data lives exclusively in the clients IndexedDB."* The diff contradicts this:

- `src/models/report_task.py` stores `csv_content` on the model:
  ```python
  self.csv_content: Optional[str] = task_data.get("csv_content")
  ...
  "csv_content": self.csv_content,
  ```
- `src/repositories/mongo_task_repository.py` writes `csv_content` to MongoDB:
  ```python
  if csv_content is not None:
      update_data["csv_content"] = csv_content
  ```
- `src/blueprints/auth.py` (`download_csv`) reads `task.csv_content` from MongoDB and serves it.

The project-specific rule **Privacy: Server must NOT persist CSV report content or tax summaries** is being broken by this change. Either:
1. Remove `csv_content` from `ReportTask` and persist *only* `csv_hash` (the documented design), and make the client responsible for retaining the CSV in IndexedDB for re-download — **or** —
2. Update `ARCHITECTURE.md` / `README.md` to reflect the actual policy, and get explicit sign-off, since this is a documented user-facing privacy guarantee.

#### 🟡 Logic / Security

- `src/blueprints/auth.py`: the upload endpoint computes `csv_hash` for integrity but never compares the stored hash against a client-supplied hash on download — the integrity check described in `ARCHITECTURE.md` is one-directional only. If you keep `csv_content` in Mongo, at least verify hash equality before serving.
- The webhook signature path uses `X-Signature` header; confirm this matches Lemon Squeezy's actual header name (`X-Signature` is correct for Lemon Squeezy — OK, but worth a comment).
- `tests/unit/test_webhook_idempotency.py` patches `src.app.payment_service.user_repository` but the production code accesses the repo via the service, not the module-level attribute. Verify the patch target; otherwise this test passes without exercising the real path.

#### 🟡 Tests

- New tests added (good), but no unit test exists for the refactored `mongo_task_repository` `create_task_metadata` / `update_task_status` paths. Add coverage under `tests/unit/` for both the success path and the privacy rule (assert that no `csv_content` is written when the zero-retention policy is enforced).
- The download-race-condition test should also assert that `csv_content` is **never** written by the server, to lock in the privacy guarantee.

#### 🟢 Minor / Style

- `tests/integration/test_end_to_end.py` still contains `print(...)` debug noise (`print("\nRow differences found:")` etc.). Replace with `LoggerService` per the logging rule.
- `src/blueprints/auth.py` returns `{"error": "Geçersiz dosya formatı..."}` — user-facing strings must be English unless localization is added; this is Turkish. Same applies to any other tr-string errors in the diff.
- Commit messages are clean and descriptive — good.

#### ✅ Looks good

- `Decimal` is used consistently for monetary values (no `float` introduced for tax/money).
- Blueprints live under `src/blueprints/`, not inline in `app.py`.
- `ConcurrencyService` is used for parallel work.
- `LoggerService` is used for errors/info.
- JS is split into focused ES modules with proper single-responsibility boundaries.

Please address the **Blocking** privacy item first — it directly violates a documented user-facing promise and a project-specific review rule.
...
7. **Real PR reviews catch real problems** — the privacy violation in TaxCalculator#13 would have been missed by manual review alone.
